...
Note that `fantaxtic` is essentially a wrapper around [`ggnested`](https://github.com/gmteunisse/ggnested), with some accessory functions to identify top taxa and to ensure that the plot is useful. Thus, the output is `ggplot2` object, and can be manipulated as such.


**Keywords: nested bar plot, phyloseq, taxonomy, most abundant taxa, multiple levels, shades, tins, gradient, 16S, ITS ,18S, microbiome, amplicon sequencing, metabarcoding**
...
### `ggnested`
Finally, the melted phyloseq object can be plotted using `ggnested` and `ggplot`. This gives full control over all aspects of the plot, including details such as the bar width. For more details, see the documentation of [`ggnested`](https://github.com/gmteunisse/ggnested).
